...
/**
 * This is a build task that runs in bnd. 
 */
public class GuardInfoTask implements AnalyzerPlugin {
	private static final String PATH_PREFIX = "enroute/sine-qua-non/";
	...
	public boolean analyzeJar(Analyzer analyzer) throws Exception {
		try {
			...
			for (Entry<String, Attrs> e : p.entrySet()) {
				...
				case "def":
					guard.name = e.getValue().get("profile");
					guard.version = new Version(e.getValue().getVersion());
					break;
			...
			if ( guard.name == null) {
				analyzer.error("No def;profile=p;version=v clause in %s=%s", COMMAND, cmd);
			}
			...
			EmbeddedResource resource = new EmbeddedResource(
					bout.toByteArray(), 0);
			analyzer.getJar().putResource(PATH_PREFIX + guard.name, resource);
			return false;
		} catch (Throwable e) {
			e.printStackTrace();
			return false;
